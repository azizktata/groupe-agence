...
import { UiHotelAvail } from "@/mappers/mapSabreHotelAvailToUi";
import { Star, MapPin, Building2, ArrowRight, Sparkles, Navigation } from "lucide-react";
import { Button } from "@/components/ui/button";
import { getHotelImage } from "@/lib/hotelImages";

type HotelAvailCardProps = {
  hotel: UiHotelAvail;
};
...
const HotelAvailCard = ({ hotel }: HotelAvailCardProps) => {
  const router = useRouter();
  const displayImage = hotel.image || getHotelImage(hotel.id, "card");

  const handleClick = () => {
    if (hotel.rooms.length > 0) {
      sessionStorage.setItem(
        `hotel-avail-rooms-${hotel.id}`,
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-3xl shadow-sm hover:shadow-xl hover:border-[var(--brand-primary)]/30 transition-all duration-300 overflow-hidden group">
          ...
          {hotel.distance !== null && (
            <div className="flex items-center gap-1 bg-white/90 backdrop-blur-sm px-2.5 py-1.5 rounded-full shadow-sm">
              ...
              <span className="text-xs font-semibold text-slate-600">
                {hotel.distance.toFixed(1)} {hotel.distanceUnit || "km"}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="p-5">
        ...
        {/* Price section */}
        {hotel.averageNightlyRate !== null && (
          <div className="flex items-baseline gap-1.5 mb-4">
            <span className="text-2xl font-bold text-[var(--brand-primary)]">
              {formatCurrency(hotel.averageNightlyRate, hotel.currencyCode)}
            </span>
        ...
        {hotel.averageNightlyRate === null && (
          <div className="mb-4">
            <span className="text-sm text-slate-400 italic">Prix non disponible</span>
          </div>
        )}
